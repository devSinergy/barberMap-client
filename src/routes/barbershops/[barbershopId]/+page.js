...
export async function load({ params }) {
    ...
    try {
        const [detailStore, services,calendar,reviews,detailreviews,appoitmens] = await Promise.all([
            getStoreOne(id),
            getServices(id),
            getCalendar(id),
            getStoreReview(id),
        ...
        return { detailStore, services,calendar,reviews,detailreviews,appoitmens };
    } catch (error) {
