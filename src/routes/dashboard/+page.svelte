...
// @ts-nocheck
    ...
    import { showAppoitmens,deleteApppoitmens ,confirmAppoitmen} from "$lib/comunications/endpoints/appoitmensRoutes";
    ...
    try {
        const message = await confirmAppoitmen(id, newStatus);
    ...
    <section class="flex-1 p-4">
        ...
        {:else}
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                ...
                {#each filteredAppointments as { _id, clientname, date, hour, hourfinish, barberid,status }}
                    <div class="bg-white flex flex-col items-center p-4 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300">
                        ...
                        {#if status === 'pendiente'}
                            <div class="flex gap-2 mt-4">
                                <button class="bg-green-600 text-white px-3 py-1 rounded" on:click={() => updateStatus(_id, 'confirmada')}>
                                    Confirmar
                                </button>
                                
                            </div>
                        {:else if status === 'confirmada'}
                            <p class="text-green-600 font-bold">Cita confirmada</p>
                        {/if}
